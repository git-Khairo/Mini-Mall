            ...
            <div class="text-center space-y-2 sm:text-left">
                ...
                @if ($user->activity == 'active')
                    <form action="{{ route('blockUser', ['id' => $user->id]) }}" method="post">
                        @csrf
                        <button type="submit" class="px-4 py-1 text-sm text-red-400 font-semibold rounded-full border border-red-400 hover:text-white hover:bg-red-400 hover:border-transparent">Block</button>
                    </form>  
                @else
                    <form action="{{ route('unBlockUser', ['id' => $user->id]) }}" method="post">
                        @csrf
                        <button type="submit" class="px-4 py-1 text-sm text-red-400 font-semibold rounded-full border border-red-400 hover:text-white hover:bg-red-400 hover:border-transparent">Unblock</button>
                    </form> 
                @endif
            </div>
        </div>
